 ...
 * Time: 13:04
 */
namespace xpbl4\export\models;

use xpbl4\export\ExcelWriter;
use xpbl4\export\ExportInterface;
use Yii;
use yii\base\Model;
		...
		if ($this->checkHeaders()) {
			if ($this->process) {
				...
				for ($i = max(1, $this->offset); $i < min($this->count, max(1, $this->offset) + $this->limit); ++$i) {
					...
					if (!$emptyRow) {
						if ($model->export($reader, $i, $dataRow)) {
						...
						if ($max_time * 0.8 < $this->_time['end'] - $this->_time['start']) {
							...
							$this->addError('timeout', 'Export run other timeout. Start export in background.');
							return false;
						}
					}
				}
